...
class Bvb_Grid_Deploy_Table extends Bvb_Grid_DataGrid {
	...
	function pagination() {
		...
		if (isset ( $this->info ['ajax'] ) && $this->info ['ajax'] !== false) {
			$pag = ($actual == 1) ? '<strong>1</strong>' : "<a href=\"javascript:gridAjax('{$this->info['ajax']}','$url/start/0')\">1</a>";
		} else {
		...
		if ($npaginas > 5) {
			...
			for($i = $in + 1; $i < $fin; $i ++) {
				if (isset ( $this->info ['ajax'] ) && $this->info ['ajax'] !== false) {
					$pag .= ($i == $actual) ? "<strong> $i </strong>" : " <a href=javascript:gridAjax('{$this->info['ajax']}','$url/start/" . (($i - 1) * $ppagina) . "')> $i </a>";
				} else {
		...
		if (isset ( $this->info ['ajax'] ) && $this->info ['ajax'] !== false) {
			$pag .= ($actual == $npaginas) ? "<strong>" . $npaginas . "</strong>" : " <a href=\"javascript:gridAjax('{$this->info['ajax']}','$url/start/" . (($npaginas - 1) * $ppagina) . "')\">$npaginas</a> ";
		...
		if ($actual != 1) {
			...
			if (isset ( $this->info ['ajax'] ) && $this->info ['ajax'] !== false) {
				$pag = " <a href=\"javascript:gridAjax('{$this->info['ajax']}','$url/start/0')\">" . $this->__ ( 'First' ) . "</a>&nbsp;&nbsp;<a href=\"javascript:agridAjax('{$this->info['ajax']}','$url/start/" . (($actual - 2) * $ppagina) . "')\">" . $this->__ ( 'Previous' ) . "</a>&nbsp;&nbsp;" . $pag;
		...
		if ($actual != $npaginas) {
			if (isset ( $this->info ['ajax'] ) && $this->info ['ajax'] !== false) {
				...
				$pag .= "&nbsp;&nbsp;<a href=\"javascript:gridAjax('{$this->info['ajax']}','$url/start/" . ($actual * $ppagina) . "')\">" . $this->__ ( 'Next' ) . "</a> <a href=\"javascript:gridAjax('{$this->info['ajax']}','$url/start/" . (($npaginas - 1) * $ppagina) . "')\">" . $this->__ ( 'Last' ) . "&nbsp;&nbsp;</a>";
			} else {
		...
		if ($npaginas > 1 && isset ( $this->info ['limit'] ) && ( int ) @$this->info ['limit'] == 0) {
			...
			if ($npaginas < 100) {
				...
				if (isset ( $this->info ['ajax'] ) && $this->info ['ajax'] !== false) {
					$f = "<select id=\"idf\" onchange=\"javascript:gridAjax('{$this->info['ajax']}','{$url}/start/'+this.value)\">";
				} else {
	...
	function _validateFilters($filters) {
		...
		if (is_array ( $filters )) {
			...
			$tab = parent::_getDescribeTable ( $this->data ['table'] );
	...
	function deploy() {
		...
		if ((isset ( $this->info ['double_tables'] ) && $this->info ['double_tables'] == 1) || (@$this->ctrlParams ['edit'] != 1 && @$this->ctrlParams ['add'] != 1)) {
			...
			if (($this->formPost == 1 && $this->formSuccess == 1) || $this->formPost == 0) {
				...
				$grid .= self::buildTitlesTable ( parent::_buildTitles () );
				$grid .= self::buildFiltersTable ( parent::_buildFilters () );
				$grid .= self::buildGridTable ( parent::_buildGrid () );
				$grid .= self::buildSqlexpTable ( parent::_buildSqlExp () );
